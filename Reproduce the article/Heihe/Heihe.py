...
def plot_under_mae(temp,model,time_arr,out_path):
    ...
    plt.plot(time_arr*time_arr.shape[0], temperature_smooth, label='Observation', color='blue')
    plt.plot(time_arr*time_arr.shape[0],temperature_smooth_val,label='Predicted', color='red')  # linestyle='dashed'
...
if __name__ == "__main__":
    model = torch.load(r"./Heihe.pth",weights_only=False)
    ...
    with torch.no_grad():
        ...
        for index,temp_number in enumerate(colname):
            if index == 0 or index == 1:
                ...
                outpath = temp_number + '.svg'
                plot_under_mae(str_to_number(temp_number), model, time_nor,outpath)
