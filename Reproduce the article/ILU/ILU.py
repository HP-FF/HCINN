...
def plot_under_mae(temp,model,time_arr,out_path):
    ...
    with torch.no_grad():
        u_pred = model(t,x).cpu().numpy().reshape(695)
    ...
    plt.plot(time_arr*694, temperature_smooth, label='Observation', color='blue')
    plt.plot(time_arr*694, temperature_smooth_val,label='Predicted', color='red')
...
if __name__ == "__main__":
    model = torch.load(r"./ILU.pth",weights_only=False)
