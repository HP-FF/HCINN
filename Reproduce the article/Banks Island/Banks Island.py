...
def plot_under_mae(temp,model,time_arr,out_path):
    if temp == 0.01:
        x = 0*torch.ones(time_arr.shape[0],1).float().to(device)
    else:
        x = (temp-0.01)/(meter-0.01) * torch.ones(time_arr.shape[0],1).float().to(device)  # Because 0.01 is approximated to 0m in data.
    t = torch.from_numpy(time_arr).float().reshape(time_arr.shape[0],1).to(device)
    with torch.no_grad():
        u_pred = model(t,x).cpu().numpy().reshape(1038)
    ...
    plt.plot(time_arr*1037, temperature_smooth, label='Observation', color='blue')
    plt.plot(time_arr*1037, temperature_smooth_val,label='Predicted', color='red')
...
if __name__ == "__main__":
    model = torch.load(r"./Banks Island.pth",weights_only=False)
