...
def plot_under_mae(temp,model,time_arr,out_path):
    if temp == 0:
        ...
        x = (temp-0.02)/(meter-0.02) * torch.ones(time_arr.shape[0],1).float().to(device)  # Because 0.02 is approximated to 0m in data.
...
if __name__ == "__main__":
    model = torch.load(r"./Tanggula.pth",weights_only=False)
    true_excel = pd.read_excel(r"./val.xlsx")
    train_excel = pd.read_excel(r"./train.xlsx")
    Total_excel = pd.read_excel(r"./Total_3m.xlsx")
